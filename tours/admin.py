...
class TourAdmin(admin.ModelAdmin):
    ...
    search_fields = ('name',)
    filter_horizontal = (
        'tour_photo_gallery', 'free_services', 'additional_options', 'lunch',
        'route')
    exclude = ('author',)
    ...
    def has_change_permission(self, request, obj=None):
        ...
        if obj is not None and request.user.groups.filter(
                name='Tour Provider').exists():
    ...
    def save_model(self, request, obj, form, change):
        if not change or obj.author is None:
            # Automatically set the author to the logged-in user if this is a new object or if author is not set
